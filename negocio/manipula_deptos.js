function matPrimas(){
  var mat = document.getElementById("matPrima");
  //var caja = document.getElementsByClassName("cont_caja");
  var m = document.getElementById("mascota");
  var e = document.getElementById("electronica");
  if (mat.style.display === "none") {
	mat.style.display = "block";
	//caja.style.display = "none";
	m.style.display = "none";
	e.style.display = "none";
  } else {
	mat.style.display = "none";
	m.style.display = "none";
	e.style.display = "none";
	//caja.style.display = "none";
  }
}
function mascotas(){
  var mas = document.getElementById("mascota");
  //var caja = document.getElementsByClassName("cont_caja");
  var m = document.getElementById("matPrima");
  var e = document.getElementById("electronica");
  if (mas.style.display === "none") {
	mas.style.display = "block";
	//caja.style.display = "none";
	m.style.display = "none";
	e.style.display = "none";
  } else {
	mas.style.display = "none";
	m.style.display = "none";
	e.style.display = "none";
	//caja.style.display = "none";
  }
}
function electronicos() {
  var ele = document.getElementById("electronica");
  //var caja = document.getElementsByClassName("cont_caja");
  var mt = document.getElementById("matPrima");
  var ms = document.getElementById("mascota");
  if (ele.style.display === "none") {
	ele.style.display = "block";
	//caja.style.display = "none";
	mt.style.display = "none";
	ms.style.display = "none";
  } else {
	ele.style.display = "none";
	mt.style.display = "none";
	ms.style.display = "none";
	//caja.style.display = "none";
  }
}